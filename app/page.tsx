...
const isMasterProgram = programDays.some((d: any) => d.isPeriodized === true || d.assignedByCoach === 'riprendi@gmail.com');
  ...
  const handleLogSet = (e: React.SyntheticEvent) => {
    ...
    // --- TRIGGER NOTIFICA COACH QUANDO L'ATLETA CONCLUDE LA SETTIMANA/FASE ---
  const triggerCoachNotificationOnPhaseComplete = async (athleteName: string, phaseName: string, week: number) => {
    try {
      await fetch('/api/notify-coach', {
        ...
        body: JSON.stringify({
          ...
          completedWeek: week,
          nextStepProposal: week >= 4 
            ? 'Transizione alla Macro-Fase Successiva di Emilio They' 
            : `Avanzamento a Settimana ${week + 1} (Intensificazione o Deload)`,
        }),
      });
    } catch (err) {
      console.error('Invio notifica coach fallito:', err);
    }
  };
...
{programDays.map((day) => {
                ...
                return (
                  <div key={day.id} className="bg-zinc-900/60 rounded-2xl border border-white/5 overflow-hidden transition-all shadow-lg">
                      ...
                      <div className="p-4 sm:p-5 border-t border-white/5 space-y-4 bg-black/20">
                        ...
                        <div className="space-y-2">
                          {day.exercises.length === 0 ? (
                            <p className="text-xs text-zinc-500 italic py-2 text-center">Nessun esercizio inserito in questa giornata.</p>
                          ) : (
